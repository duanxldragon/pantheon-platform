...
import { SimplePagination } from './simple_pagination';
  ...
  return (
    ...
    >
      ...
      <Table className="min-w-max" style={{ minWidth: tableMinWidth }}>
        <TableHeader>
          <TableRow className="border-b border-slate-200/80 bg-gradient-to-r from-slate-50/95 via-slate-50/88 to-white/90">
            {selectable ? (
              <TableHead
          ...
          {loading ? (
            ...
            <TableRow>
              <TableCell
                ...
                className="py-12"
              >
                <div className="mx-auto flex max-w-sm flex-col items-center rounded-[24px] border border-dashed border-slate-200/80 bg-slate-50/85 px-6 py-8 text-center shadow-sm shadow-slate-200/40">
                  <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">
                    {zh ? 'EMPTY' : 'EMPTY'}
                  </div>
                  <div className="mt-3 text-sm font-semibold text-slate-900">
                    {zh ? '当前没有可展示的数据' : 'No records to show right now'}
                  </div>
            ...
            data.map((item) => {
              ...
              return (
                <TableRow
                  key={key}
                  className={cn(
                    'cursor-pointer border-slate-100/80 transition-all duration-200',
                    isSelected ? 'bg-primary/5 shadow-[inset_0_1px_0_rgba(59,130,246,0.08)]' : isHovered ? 'bg-slate-50/85' : 'bg-transparent',
                  )}
      ...
      {pagination ? (
        <div className="border-t border-slate-200/80 bg-gradient-to-r from-slate-50/88 via-white/80 to-slate-50/88 px-5 py-4">
          <SimplePagination
