...
interface ConnectionConfigStepProps {
  ...
  recommendedDatabaseName?: string;
  recommendedSQLitePath?: string;
      ...
      <div className="mx-auto max-w-2xl space-y-6 rounded-3xl border border-slate-100 bg-slate-50/50 p-8">
        {isSqlite ? (
              ...
              <Input
                ...
                onChange={(event) =>
                  onChange({
                    ...
                    database: config.database || recommendedDatabaseName || '',
                  })
                }
                placeholder={recommendedSQLitePath || '/data/pantheon_tenant_acme.db'}
          ...
          <>
            ...
            <FormField
              label={zh ? '数据库名' : 'Database Name'}
              required
              description={
                zh
                  ? '推荐使用 pantheon_tenant_<tenant_code>，例如 pantheon_tenant_acme。'
                  : 'Use pantheon_tenant_<tenant_code>, for example pantheon_tenant_acme.'
              }
            >
              <div className="relative">
                ...
                <Input
                  ...
                  placeholder={recommendedDatabaseName || 'pantheon_tenant_acme'}
                  className="h-12 rounded-xl bg-white pl-9"
                />
              </div>
            </FormField>
