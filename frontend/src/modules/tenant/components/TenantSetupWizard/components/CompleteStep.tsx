...
import { CheckCircle2, Copy, LayoutDashboard, PartyPopper } from 'lucide-react';
...
export function CompleteStep({ onComplete, managed = false, targetTenantName, result }: CompleteStepProps) {
  ...
  const copyText = useMemo(() => {
    const lines = [
      ...
      bootstrap?.initialPassword ? `${zh ? '初始密码' : 'Initial password'}: ${bootstrap.initialPassword}` : '',
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      systemNotification.success(zh ? '账号信息已复制' : 'Credentials copied');
    } catch (error) {
    ...
    <div className="animate-in fade-in zoom-in flex flex-col items-center py-12 text-center duration-700">
      <div className="relative mb-10">
        ...
        <div className="absolute -right-2 -top-2 flex gap-1">
          {[1, 2, 3].map((index) => (
            <div key={index} className="h-2 w-2 animate-ping rounded-full bg-amber-400" style={{ animationDelay: `${index * 0.2}s` }} />
          ))}
        </div>
      </div>
      ...
      <p className="mb-8 max-w-2xl font-medium leading-relaxed text-slate-500">
        {zh
          ? managed
      ...
      <div className="mb-8 grid w-full max-w-3xl grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
          ...
          <div className="text-left">
            ...
            <p className="text-sm font-medium text-emerald-900">
              {zh ? `已引导 ${bootstrap?.permissionCount ?? 0} 个权限模板` : `${bootstrap?.permissionCount ?? 0} permission templates bootstrapped`}
            </p>
          </div>
        </div>
      </div>
      ...
      {bootstrap?.adminUsername ? (
        <div className="mb-10 w-full max-w-2xl rounded-3xl border border-slate-200 bg-slate-50 p-6 text-left shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-4">
            <div>
              <h3 className="text-lg font-bold text-slate-900">{zh ? '默认管理员账号' : 'Default Admin Account'}</h3>
              <p className="text-sm text-slate-500">
                {zh ? '建议首次登录后立即修改密码，并按需开启 2FA。' : 'Change the password immediately after first login and enable 2FA if needed.'}
              </p>
            </div>
            {copyText ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl bg-white p-4">
              <p className="text-xs font-semibold uppercase text-slate-500">{zh ? '账号' : 'Username'}</p>
              <p className="mt-1 break-all text-base font-bold text-slate-900">{bootstrap.adminUsername}</p>
            </div>
            <div className="rounded-2xl bg-white p-4">
              <p className="text-xs font-semibold uppercase text-slate-500">{zh ? '邮箱' : 'Email'}</p>
              <p className="mt-1 break-all text-base font-bold text-slate-900">{bootstrap.adminEmail || '-'}</p>
            </div>
            <div className="rounded-2xl bg-white p-4">
              <p className="text-xs font-semibold uppercase text-slate-500">{zh ? '默认角色' : 'Role'}</p>
              <p className="mt-1 break-all text-base font-bold text-slate-900">{bootstrap.roleCode || 'super_admin'}</p>
            </div>
            <div className="rounded-2xl bg-white p-4">
              <p className="text-xs font-semibold uppercase text-slate-500">{zh ? '初始密码' : 'Initial Password'}</p>
              <p className="mt-1 break-all text-base font-bold text-slate-900">
                {bootstrap.initialPassword || (zh ? '沿用已有管理员密码' : 'Existing admin password preserved')}
              </p>
            </div>
          </div>
        </div>
      ...
      <Button
        onClick={onComplete}
        size="lg"
        className="group h-16 rounded-2xl bg-slate-900 px-12 text-lg font-bold shadow-2xl transition-all active:scale-95 hover:bg-black"
      >
        {zh ? (managed ? '返回租户管理' : '进入管理后台') : managed ? 'Back to Tenant Management' : 'Enter Admin Console'}
