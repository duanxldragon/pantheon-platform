...
import { ConnectionConfigStep } from './components/ConnectionConfigStep';
import { DatabaseTypeStep } from './components/DatabaseTypeStep';
import { TestConnectionStep } from './components/TestConnectionStep';
...
interface TenantSetupWizardProps {
  ...
  onCompleted?: (tenantId: string) => void | Promise<void>;
}
    ...
    switch (currentStep) {
      ...
      case 'connection-config':
        return config.databaseType ? (
          <ConnectionConfigStep databaseType={config.databaseType} config={config} onChange={handleConfigChange} />
        ) : null;
