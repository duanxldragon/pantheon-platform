...
export function useLoginForm() {
  ...
  const [tenantCodeExpanded, setTenantCodeExpanded] = useState(Boolean(rememberedTenantCode));
  ...
  const showTenantCode = enableMultiTenant || Boolean(rememberedTenantCode);
  const shouldShowTenantCodeInput = showTenantCode && tenantCodeExpanded;
  ...
  useEffect(() => {
    if (!showTenantCode) {
      setTenantCodeExpanded(false);
      return;
    }

    if (loginRequiresTenantCode || rememberedTenantCode) {
      setTenantCodeExpanded(true);
    }
  }, [loginRequiresTenantCode, rememberedTenantCode, showTenantCode]);
  ...
  useEffect(() => {
    const normalizedTenantCode = normalizeTenantCode(tenantCode);
    if (!shouldShowTenantCodeInput || !normalizedTenantCode) {
  ...
  }, [shouldShowTenantCodeInput, tenantCode]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const result = await login(
        ...
        password,
        remember,
        shouldShowTenantCodeInput ? normalizedTenantCode : undefined,
      );
      ...
      if (showTenantCode && !shouldShowTenantCodeInput) {
        setTenantCodeExpanded(true);
      }
