...
import { authApi, type EnableTwoFactorResponse, type SessionInfo } from '../api/authApi';
...
function resolveQrCodeUrl(data: EnableTwoFactorResponse | null): string {
  return data?.qrCodeUrl || data?.qr_code_url || '';
}

function resolveBackupCodes(data: EnableTwoFactorResponse | null): string[] {
  return data?.backupCodes || data?.backup_codes || [];
}
...
function mapSession(session: SessionInfo, zh: boolean): DeviceSession {
  const name = session.device_name || (zh ? '未知设备' : 'Unknown device');
  return {
    id: session.jti,
    name,
    location: zh ? '未知地区' : 'Unknown',
    ip: session.ip_address || '-',
    lastActive: formatRelativeTime(session.last_active, zh),
...
export function useTwoFactorSettings() {
  ...
  const handleToggle2FA = async (checked: boolean) => {
    ...
    const password = window.prompt(zh ? '请输入当前密码以关闭双因素认证' : 'Enter password to disable 2FA');
    if (!password) {
      return;
    }
