...
import type { SettingsField, SettingsSection, SettingValue } from '../hooks/use_settings_logic';

interface SettingsFormGroupProps {
  section: SettingsSection;
  values: Partial<Record<string, SettingValue>>;
  onChange: (key: string, value: SettingValue) => void;
  onInspect?: (field: SettingsField) => void;
  copy: {
    required: string;
    enabled: string;
    disabled: string;
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex-1 space-y-1">
            <div className="flex items-center gap-2">
              ...
              {isModified ? (
                <Badge variant="warning" className="h-4 px-1.5 text-[9px] font-bold uppercase">
                  {copy.modified}
                </Badge>
              ) : null}
          ...
          <div className="flex min-w-[280px] flex-none items-start justify-end gap-3">
            {onInspect ? (
              <Button
                type="button"
                variant="mono"
                ...
                aria-label={`${copy.viewDetail}: ${item.label}`}
              >
