...
- `Monitor DB` 用于监控快照等可选历史数据
- `Storage Provider` 负责本地 / 对象存储适配
- `Tenant Migrators` 负责把模块能力标准化接入租户初始化链路
...
- 项目入口与总体定位：`README.md`
- 平台级设计与治理：`docs/`
- 后端工程总览与专题实现：`backend/BACKEND_GUIDE.md`、`backend/docs/`
- 前端工程总览与专题实现：`frontend/FRONTEND_GUIDE.md`、`frontend/docs/`
- 动态视图运行时与初始化分层：`frontend/docs/DYNAMIC_VIEW_RUNTIME.md`、`frontend/docs/system/SYSTEM_INITIALIZATION_LAYERS.md`
- 生成产物与接口文档：`backend/api/swagger/`
