...
- 涉及中文文案、Markdown、配置文件或出现乱码时，先看 `docs/governance/ENCODING_AND_LINE_ENDINGS.md`
...
- 不要忽略中文乱码；看到异常字符、问号替代或 mojibake 时，先判断是否为编码问题
- 新增或修改文本文件时，默认保持 `UTF-8`；不要无故改整文件换行或编码
- 发现乱码时，先检查文件实际编码、编辑器打开编码和 `.editorconfig` / `.gitattributes`，不要直接整段重写
- 涉及规则、文档、i18n 文案时，提交前至少抽查一次中文是否可正常显示
...
- 默认按“团队协作式拆解”处理任务，而不是只按“前端 / 后端”粗暴二分。
- 先按主链路判断任务属于 `auth`、`tenant`、`system` 中哪一条，再决定是否需要前端壳层、基础设施、验证角色介入。
- 涉及菜单、权限、标签页、动态视图时，必须同时检查：
  - `authStore`
  - `systemStore`
  - `uiStore`
  - `ViewRenderer`
