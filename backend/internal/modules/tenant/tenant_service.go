...
func (s *tenantService) CreateTenant(ctx context.Context, req *CreateTenantRequest) (*Tenant, error) {
	normalizedCode := normalizeTenantCode(req.Code)
	if err := validateTenantCode(normalizedCode); err != nil {
		return nil, err
	}

	if _, err := s.tenantDAO.GetByCode(ctx, normalizedCode); err == nil {
		return nil, ErrTenantCodeAlreadyExists
...
func (s *tenantService) GetTenantStatus(ctx context.Context, code string) (*TenantStatusResponse, error) {
	tenantRecord, err := s.tenantDAO.GetByCode(ctx, normalizeTenantCode(code))
	if err != nil {
		return nil, err
	}
...
func (s *tenantDatabaseService) SetupDatabase(ctx context.Context, tenantID string, req *SetupDatabaseRequest) (*SetupDatabaseResponse, error) {
	log.Printf("tenant setup start tenant_id=%s db=%s type=%s", tenantID, req.Database, req.DatabaseType)
	tenantUUID, err := uuid.Parse(tenantID)
	if err != nil {
		return nil, err
	}
	...
	log.Printf("tenant setup connected tenant_id=%s db=%s", tenantID, req.Database)
	...
	log.Printf("tenant setup config persisted tenant_id=%s config_id=%s", tenantID, config.ID.String())

	bootstrap, err := s.bootstrapTenantDatabaseWithRetry(ctx, tenantRecord, req, dbConfig, tenantDB)
	if err != nil {
		return nil, err
	}
	log.Printf("tenant setup bootstrap completed tenant_id=%s seeded=%v", tenantID, bootstrap.Seeded)
	...
	log.Printf("tenant setup finished tenant_id=%s", tenantID)
...
func (s *tenantDatabaseService) bootstrapTenantDatabase(
	...
	if err := tenantDB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		log.Printf("tenant bootstrap transaction start tenant_id=%s", tenantRecord.ID.String())
		var err error
	...
	log.Printf("tenant bootstrap transaction committed tenant_id=%s role_created=%v admin_created=%v", tenantRecord.ID.String(), roleCreated, adminCreated)

	tenantCtx := context.WithValue(ctx, "tenant_db", tenantDB)
	tenantCtx = context.WithValue(tenantCtx, "tenant_id", tenantRecord.ID.String())

	if s.authzSvc != nil {
		log.Printf("tenant bootstrap authz bind role tenant_id=%s user_id=%s role_id=%s", tenantRecord.ID.String(), adminRecord.ID.String(), roleRecord.ID.String())
		if err := s.authzSvc.AddRoleForUser(tenantCtx, adminRecord.ID.String(), roleRecord.ID.String()); err != nil {
			return nil, err
		}
		log.Printf("tenant bootstrap authz bind role done tenant_id=%s", tenantRecord.ID.String())
		for _, permissionRule := range permissionRules {
			log.Printf("tenant bootstrap authz add permission tenant_id=%s role_id=%s resource=%s action=%s", tenantRecord.ID.String(), roleRecord.ID.String(), permissionRule.Resource, permissionRule.Action)
		...
		log.Printf("tenant bootstrap authz permissions done tenant_id=%s count=%d", tenantRecord.ID.String(), len(permissionRules))
	}
...
func (s *tenantDatabaseService) bootstrapTenantDatabaseWithRetry(
	ctx context.Context,
	tenantRecord *Tenant,
	req *SetupDatabaseRequest,
	dbConfig *database.DBConfig,
	...
	if !isClosedDatabaseError(err) || tenantRecord == nil || dbConfig == nil {
		return nil, err
	}

	log.Printf("tenant bootstrap retrying after closed database tenant_id=%s err=%v", tenantRecord.ID.String(), err)
	s.dbManager.RemoveTenant(tenantRecord.ID)
	if reconnectErr := s.dbManager.ConnectTenant(ctx, dbConfig, req.Password, tenantRecord.Code); reconnectErr != nil {
		return nil, err
	}

	refreshedTenantDB := s.dbManager.GetTenantDB(tenantRecord.ID)
	if refreshedTenantDB == nil {
		return nil, err
	}

	return s.bootstrapTenantDatabase(ctx, tenantRecord, req, refreshedTenantDB)
}
...
func isTenantCodeConflict(err error) bool {
	if err == nil {
		return false
	}
