...
func (h *AuthHandler) RegisterRoutes(group *gin.RouterGroup, authMiddleware gin.HandlerFunc) {
	...
	{
		auth.GET("/config", h.GetPublicConfig)
		auth.POST("/login", h.Login)
		auth.POST("/2fa/login", h.VerifyLogin2FA)
		auth.POST("/refresh", h.RefreshToken)
		auth.POST("/unlock", h.UnlockAccount)
		auth.GET("/attempts", h.GetLoginAttempts)
		auth.POST("/validate-password", h.ValidatePassword)
	}

	protectedAuth := group.Group("/auth")
	protectedAuth.Use(authMiddleware)
	{
		protectedAuth.GET("/current", h.GetCurrentUser)
	...
	{
		twoFactor.GET("/status", h.GetTwoFactorAuthStatus)
		twoFactor.POST("/enable", h.EnableTwoFactorAuth)
		twoFactor.POST("/verify", h.VerifyAndEnable2FA)
		twoFactor.POST("/disable", h.DisableTwoFactorAuth)
		twoFactor.POST("/backup-codes", h.GenerateNewBackupCodes)
		twoFactor.POST("/verify-code", h.Verify2FACode)
	}
	...
	sessions := protectedAuth.Group("/sessions")
	{
		...
		sessions.DELETE("/:jti", h.KickSession)
	}
