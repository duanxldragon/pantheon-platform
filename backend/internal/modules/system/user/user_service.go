...
func (s *userService) List(ctx context.Context, req *UserListRequest) (*PageResponse, error) {
	...
	if err != nil {
		// 如果批量查询失败，回退到逐个查询
		log.Printf("BatchGetRoles failed, falling back to individual queries: %v", err)
		rolesMap = make(map[string][]*UserRoleInfo)
		for _, u := range users {
			roles, _ := s.userDAO.GetRoles(ctx, u.ID.String())
			rolesMap[u.ID.String()] = roles
		}
	}
